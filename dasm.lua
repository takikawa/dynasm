--binding to the DynASM encoding engine (Cosmin Apreutesei, public domain)
...
function M.new(actionlist, externnames, sectioncount, globalcount, externget, globals)
	...
	ffi.gc(state, function(state)
		M.free(state)
		globals = nil --pin it (don't rely on the user doing it)
	end)
...
if not ... then --demo
	local dasm = M
	--local actions = ffi.new('const uint8_t[5]', {102,184,5,0,255}) --mov ax, 5
